...
The feature begins with a farm boundary/polygon for the input farm that is passed by the API, it matches the polygon with the Digital Elevation Model (DEM) raster, which is converted into a slope raster to identify terrain steepness. A grid of planting points is generated to define the planting areas; the gird is then rotated by potential angles to determine the orientation that maximizes sapling count. Finally, slope rules are applied to remove planting points on the terrain that is too steep, and the estimator/main algorithm outputs the final sapling count and optimal rotation angle of the input farm, which is passed back to the API.
...
* Generates a regular grid by applying spacing rules and farm polygon bounds.
...
* Tests rotation angles from 0° to 90° by rotating the grid around the farm centroid.
...
* Removes points with slope values above the maximum threshold (15°).
